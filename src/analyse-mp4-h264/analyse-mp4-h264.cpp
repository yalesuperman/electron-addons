...
private:
  ...
  int add_nalu_to_analysedata(uint8_t nal_type, int32_t nal_size, const uint8_t *nal_data, int pts = INT_MAX, int dts = INT_MAX, bool add_start_code = false);
public:
...
int AnalyseMp4::h264_mp4toannexb(AVPacket *input_packet) {
  ...
  do
  {
    ...
    add_nalu_to_analysedata(nal_type, nal_size, buf, input_packet->pts, input_packet->dts, true);
...
int AnalyseMp4::add_nalu_to_analysedata(uint8_t nal_type, int32_t nal_size, const uint8_t *nal_data, int pts, int dts, bool add_start_code) {
  json packet;
  json packetData;
  ...
  if (pts != INT_MAX) packet["pts"] = pts;
  if (dts != INT_MAX) packet["dts"] = dts;
